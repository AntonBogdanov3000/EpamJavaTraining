...
public class CheckCarCommand implements Command {
    ...
    public void execute(){
        ...
        for (int i=0; i< Creator.list.size(); i++){
            System.out.println(Creator.list.get(i).getVehicle() + carService.chekOil(Creator.list.get(i)));
            System.out.println(Creator.list.get(i).getVehicle() + carService.checkPressure(Creator.list.get(i)));
        }
    }
}
